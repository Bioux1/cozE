    ...
    async def download(self, ctx, post = None):
        if post:
            try:
                ...
                response = await ctx.send("Downloading...")
            ...
            if submission.is_video:
                ...
                await response.edit(content = f"{ctx.author.mention}, here is the video:", attachments = [discord.File(file)])
                os.remove(file)
            elif submission.is_reddit_media_domain:
                await response.edit(content = f"{ctx.author.mention}, {submission.url}")
            else:
                await response.edit(content = f"{ctx.author.mention}, post does not contain any media.")
        else:
